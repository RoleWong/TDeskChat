...
#import "TDesk_TUICameraView.h"
#import "TDesk_TUICaptureImagePreviewController.h"
#import "TDesk_TUICaptureVideoPreviewViewController.h"

#import <TDeskCommon/TDesk_TIMCommonModel.h>
#import <TDeskCommon/TDesk_TIMDefine.h>
#import "TDesk_TUICameraManager.h"
#import "TDesk_TUICaptureTimer.h"
#import "TDesk_TUIMotionManager.h"
#import "TDesk_TUIMovieManager.h"

@interface TDeskCameraViewController () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, TDeskCameraViewDelegate> {
...
@property(nonatomic, strong) TDeskCameraView *cameraView;
@property(nonatomic, strong) TDeskMovieManager *movieManager;
@property(nonatomic, strong) TDeskCameraManager *cameraManager;
@property(nonatomic, strong) TDeskMotionManager *motionManager;
...
@implementation TDeskCameraViewController

- (instancetype)init {
    self = [super init];
    if (self) {
        _motionManager = [[TDeskMotionManager alloc] init];
        _cameraManager = [[TDeskCameraManager alloc] init];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.cameraView = [[TDeskCameraView alloc] initWithFrame:self.view.bounds];
...
- (void)focusAction:(TDeskCameraView *)cameraView point:(CGPoint)point handle:(void (^)(NSError *))handle {
...
- (void)exposAction:(TDeskCameraView *)cameraView point:(CGPoint)point handle:(void (^)(NSError *))handle {
    NSError *error = [_cameraManager expose:[self activeCamera] point:point];
    handle(error);
}

- (void)autoFocusAndExposureAction:(TDeskCameraView *)cameraView handle:(void (^)(NSError *))handle {
    NSError *error = [_cameraManager resetFocusAndExposure:[self activeCamera]];
    handle(error);
}

- (void)flashLightAction:(TDeskCameraView *)cameraView handle:(void (^)(NSError *))handle {
...
- (void)torchLightAction:(TDeskCameraView *)cameraView handle:(void (^)(NSError *))handle {
...
- (void)swicthCameraAction:(TDeskCameraView *)cameraView handle:(void (^)(NSError *))handle {
    NSError *error;
...
- (void)takePhotoAction:(TDeskCameraView *)cameraView {
    ...
    [_imageOutput captureStillImageAsynchronouslyFromConnection:connection
                                              completionHandler:^(CMSampleBufferRef _Nullable imageDataSampleBuffer, NSError *_Nullable error) {
                                                ...
                                                TDeskCaptureImagePreviewController *vc = [[TDeskCaptureImagePreviewController alloc] initWithImage:image];
...
- (void)startRecordVideoAction:(TDeskCameraView *)cameraView {
     ...
     */
    _movieManager = [[TDeskMovieManager alloc] init];
    _recording = YES;
    ...
    [_movieManager stop:^(NSURL *_Nonnull url, NSError *_Nonnull error) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (duration < self.videoMinimumDuration) {
            ...
            TDeskCaptureVideoPreviewViewController *videoPreviewController = [[TDeskCaptureVideoPreviewViewController alloc] initWithVideoURL:url];
...
- (void)showErrorStr:(NSString *)errStr {
    [TDeskTool makeToast:errStr duration:1 position:CGPointMake(self.view.bounds.size.width / 2.0, self.view.bounds.size.height / 2.0)];
}
