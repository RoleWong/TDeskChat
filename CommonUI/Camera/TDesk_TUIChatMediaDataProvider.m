...
#import <TDeskCommon/TDesk_TUIUserAuthorizationCenter.h>
#import <TDeskCommon/TDesk_NSTimer+TUISafe.h>
#import <TDeskCore/TDesk_TUITool.h>
#import "TDesk_TUICameraViewController.h"

#define kTUIChatMediaSelectImageMax 9
@interface TDeskChatMediaDataProvider () <PHPickerViewControllerDelegate,
...
@implementation TDeskChatMediaDataProvider
...
- (void)takePicture {
    ...
    void (^actionBlock)(void) = ^(void) {
      TDeskCameraViewController *vc = [[TDeskCameraViewController alloc] init];
    ...
    if ([TDeskUserAuthorizationCenter isEnableCameraAuthorization]) {
        dispatch_async(dispatch_get_main_queue(), ^{
          actionBlock();
        });
    } else {
        if (![TDeskUserAuthorizationCenter isEnableCameraAuthorization]) {
            [TDeskUserAuthorizationCenter cameraStateActionWithPopCompletion:^{
              dispatch_async(dispatch_get_main_queue(), ^{
                actionBlock();
              });
            }];
...
- (void)takeVideo {
    ...
    if ([TDeskUserAuthorizationCenter isEnableMicroAuthorization] && [TDeskUserAuthorizationCenter isEnableCameraAuthorization]) {
        dispatch_async(dispatch_get_main_queue(), ^{
          actionBlock();
        });
    } else {
        if (![TDeskUserAuthorizationCenter isEnableMicroAuthorization]) {
            [TDeskUserAuthorizationCenter microStateActionWithPopCompletion:^{
              if ([TDeskUserAuthorizationCenter isEnableCameraAuthorization]) {
                  dispatch_async(dispatch_get_main_queue(), ^{
                    actionBlock();
                  });
...
- (void)handleImagePick:(BOOL)succ message:(NSString *)message imageData:(NSData *)imageData {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      NSString *path = [TUIKit_Image_Path stringByAppendingString:[TDeskTool genImageName:nil]];
...
- (void)transcodeIfNeed:(BOOL)succ message:(NSString *)message videoUrl:(NSURL *)url {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([self.listener respondsToSelector:@selector(onProvidePlaceholderVideoSnapshot:SnapImage:Completion:)]) {
            [self.listener onProvidePlaceholderVideoSnapshot:@"" SnapImage:image Completion:^(BOOL finished, TDeskMessageCellData * _Nonnull placeHolderCellData) {
...
- (void)transcodeIfNeed:(BOOL)succ message:(NSString *)message videoUrl:(NSURL *)url placeHolderCellData:(TDeskMessageCellData*)placeHolderCellData {
    if (succ == NO || url == nil) {
        [self handleVideoPick:NO message:message videoUrl:nil];
        return;
    }
...
- (void)handleVideoPick:(BOOL)succ message:(NSString *)message videoUrl:(NSURL *)videoUrl placeHolderCellData:(TDeskMessageCellData*)placeHolderCellData{
    ...
    NSString *videoPath = [NSString stringWithFormat:@"%@%@_%u.mp4", TUIKit_Video_Path, [TDeskTool genVideoName:nil],arc4random()];
    ...
    NSString *imagePath = [TUIKit_Video_Path stringByAppendingFormat:@"%@_%u",[TDeskTool genSnapshotName:nil],arc4random()];
...
- (void)_dealPHPickerResultFinishPicking:(PHPickerResult *)result API_AVAILABLE(ios(14)) {
    ...
    if ([itemProvoider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeImage]) {
        ...
        if ([self.listener respondsToSelector:@selector(onProvidePlaceholderVideoSnapshot:SnapImage:Completion:)]) {
            [self.listener onProvidePlaceholderVideoSnapshot:@"" SnapImage:nil Completion:^(BOOL finished, TDeskMessageCellData * _Nonnull placeHolderCellData) {
                [itemProvoider loadDataRepresentationForTypeIdentifier:(NSString *)kUTTypeMovie
                                                     completionHandler:^(NSData *_Nullable data, NSError *_Nullable error) {
                                                       dispatch_async(dispatch_get_main_queue(), ^{
