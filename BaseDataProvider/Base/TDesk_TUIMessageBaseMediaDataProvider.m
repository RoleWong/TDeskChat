...
@interface TDeskMessageBaseMediaDataProvider ()
@property(nonatomic) TDeskChatConversationModel *conversationModel;
...
@implementation TDeskMessageBaseMediaDataProvider
...
- (void)loadMediaWithMessage:(V2TIMMessage *)curMessage {
    ...
    if (self.loadMessage.status != V2TIM_MSG_STATUS_SENDING) {
        ...
        TDeskMessageCellData *data = [self.class getMediaCellData:self.loadMessage];
        if (data) {
...
- (void)loadOlderMedia {
    if (self.loadMessage.status != V2TIM_MSG_STATUS_SENDING) {
        TDeskMessageCellData *firstData = (TDeskMessageCellData *)self.medias.firstObject;
...
- (void)loadNewerMedia {
    if (self.loadMessage.status != V2TIM_MSG_STATUS_SENDING) {
        TDeskMessageCellData *lastData = (TDeskMessageCellData *)self.medias.lastObject;
    ...
    [self loadMediaMessage:self.loadMessage
        ...
        SucceedBlock:^(NSArray<V2TIMMessage *> *_Nonnull olders, NSArray<V2TIMMessage *> *_Nonnull newers) {
          ...
          for (V2TIMMessage *msg in olders) {
              TDeskMessageCellData *data = [self.class getMediaCellData:msg];
              if (data) {
