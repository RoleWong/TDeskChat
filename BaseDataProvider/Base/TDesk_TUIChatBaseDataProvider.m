...
- (void)getForwardMessageWithCellDatas:(NSArray<TDeskMessageCellData *> *)uiMsgs
    ...
    dispatch_apply(targets.count, dispatch_get_global_queue(0, 0), ^(size_t index) {
      TDeskChatConversationModel *convCellData = targets[index];
      ...
      if (!merge) {
          ...
          for (V2TIMMessage *msg in msgs) {
              ...
              if (forwardMessage) {
                  forwardMessage.isExcludedFromUnreadCount = [TDeskConfig defaultConfig].isExcludedFromUnreadCount;
                  forwardMessage.isExcludedFromLastMessage = [TDeskConfig defaultConfig].isExcludedFromLastMessage;
      ...
      [V2TIMManager.sharedInstance getUsersInfo:@[ loginUserId ]
                                           succ:^(NSArray<V2TIMUserFullInfo *> *infoList) {
                                             ...
                                             mergeMessage.isExcludedFromUnreadCount = [TDeskConfig defaultConfig].isExcludedFromUnreadCount;
                                             mergeMessage.isExcludedFromLastMessage = [TDeskConfig defaultConfig].isExcludedFromLastMessage;
                                             if (resultBlock) {
...
@implementation TDeskChatBaseDataProvider (IMSDK)
