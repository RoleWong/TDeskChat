...
#import <TDeskCore/TDesk_OfflinePushExtInfo.h>
...
@interface TDeskMessageBaseDataProvider () <V2TIMAdvancedMsgListener, V2TIMGroupListener,TDeskMessageProgressManagerDelegate>
@property(nonatomic, strong) TDeskChatConversationModel *conversationModel;
@property(nonatomic, strong) NSMutableArray<TDeskMessageCellData *> *uiMsgs_;
...
@implementation TDeskMessageBaseDataProvider
...
- (NSMutableArray<TDeskMessageCellData *> *)uiMsgs_ {
    if (_uiMsgs_ == nil) {
...
- (NSArray<TDeskMessageCellData *> *)uiMsgs {
    return _uiMsgs_;
}
...
- (void)onMessageStatusChanged:(NSNotification *)notification {
    ...
    TDeskMessageCellData *uiMsg = nil;
    BOOL isMatch = NO;
    for (uiMsg in self.uiMsgs) {
        if ([uiMsg.msgID isEqualToString:msgId]) {
            ...
            [self.dataSource dataProviderDataSourceChange:self withType:TDeskMessageBaseDataProviderDataSourceChangeTypeReload atIndex:index animation:YES];
...
- (void)onRecvNewMessage:(V2TIMMessage *)msg {
    ...
    TDeskMessageCellData *newUIMsg = newUIMsgs.lastObject;
    newUIMsg.source = Msg_Source_OnlinePush;

    if ([newUIMsg isKindOfClass:TDeskTypingStatusCellData.class]) {
        if (![TDeskChatConfig defaultConfig].enableTypingStatus) {
            return;
        }

        TDeskTypingStatusCellData *stastusData = (TDeskTypingStatusCellData *)newUIMsg;
    ...
    [self preProcessMessage:newUIMsgs
                   callback:^{
                     ...
                     [self.dataSource dataProviderDataSourceWillChange:self];
                     @autoreleasepool {
                         for (TDeskMessageCellData *uiMsg in newUIMsgs) {
                             [self addUIMsg:uiMsg];
                             [self.dataSource dataProviderDataSourceChange:self
                                                                  withType:TDeskMessageBaseDataProviderDataSourceChangeTypeInsert
...
- (NSMutableArray *)transUIMsgFromIMMsg:(NSArray *)msgs {
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        if (cellData) {
            TDeskMessageCellData *dateMsg = [self getSystemMsgFromDate:msg.timestamp];
            if (dateMsg) {
...
- (void)onRecvMessageReadReceipts:(NSArray<V2TIMMessageReceipt *> *)receiptList {
    ...
    // update TDeskMessageCellData readCount/unreadCount
    for (TDeskMessageCellData *data in self.uiMsgs) {
    ...
    [TDeskTool dispatchMainAsync:^{
      ...
      for (uiMsg in self.uiMsgs) {
          if ([uiMsg.msgID isEqualToString:msgID]) {
              ...
              TDeskSystemMessageCellData *revokeCellData = (TDeskSystemMessageCellData *)[self.class getRevokeCellData:uiMsg.innerMessage];
...
- (void)dealTypingByStatusCellData:(TDeskTypingStatusCellData *)stastusData {
...
- (void)loadMessages:(NSArray<V2TIMMessage *> *)msgs
        SucceedBlock:(void (^)(BOOL isFirstLoad, BOOL isNoMoreMsg, NSArray<TDeskMessageCellData *> *newMsgs))succeedBlock {
    NSMutableArray<TDeskMessageCellData *> *uiMsgs = [self transUIMsgFromIMMsg:msgs];
    if (uiMsgs.count == 0) {
        return;
    }
    @weakify(self);
...
- (void)processMessageSync:(NSArray<TDeskMessageCellData *> *)uiMsgs callback:(void (^)(void))callback {
    if (callback) {
        callback();
    }
}

- (void)processMessageAsync:(NSArray<TDeskMessageCellData *> *)uiMsgs {
...
- (void)getReactFromMessage:(NSArray<TDeskMessageCellData *> *)uiMsgs {
    if (uiMsgs.count == 0) {
        return;
    }
    // fetch react
...
- (void)requestForAdditionalUserInfo:(NSArray<TDeskMessageCellData *> *)uiMsgs callback:(void (^)(void))callback {
    ...
    [self requestForUserDetailsInfo:userIDList
                           callback:^(NSDictionary<NSString *, TDeskRelationUserModel *> *result) {
                             for (TDeskMessageCellData *cellData in uiMsgs) {
                                 ...
                                 for (NSString *userID in userIDList) {
                                     TDeskRelationUserModel *userInfo = [result objectForKey:userID];
...
- (void)requestForUserDetailsInfo:(NSArray *)userIDList callback:(void (^)(NSDictionary<NSString *, TDeskRelationUserModel *> *))callback {
    if (callback == nil) {
        return;
    }

    NSMutableDictionary<NSString *, TDeskRelationUserModel *> *result = [NSMutableDictionary dictionary];
            ...
            succ:^(NSArray<V2TIMGroupMemberFullInfo *> *memberList) {
              [memberList enumerateObjectsUsingBlock:^(V2TIMGroupMemberFullInfo *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                if (obj.userID.length > 0) {
                    TDeskRelationUserModel *userInfo = [[TDeskRelationUserModel alloc] init];
    ...
    [self preProcessMessage:@[ uiMsg ]
                   callback:^{
                     [TDeskTool dispatchMainAsync:^{
                       ...
                       TDeskMessageCellData *placeholderCellData = uiMsg.placeHolderCellData;
                       TDeskMessageCellData *dateMsg = nil;
                       BOOL isReSent = NO;
                       ...
                       imMsg.isExcludedFromUnreadCount = [TDeskConfig defaultConfig].isExcludedFromUnreadCount;
                       imMsg.isExcludedFromLastMessage = [TDeskConfig defaultConfig].isExcludedFromLastMessage;

                       // Update sender
                       uiMsg.identifier = [TDeskLogin getUserID];
                      ...
                      if (placeholderCellData) {
                       ...
                       uiMsg.msgID = [self.class sendMessage:imMsg
                           ...
                           Progress:^(uint32_t progress) {
                             [TDeskMessageProgressManager.shareManager appendUploadProgress:uiMsg.msgID progress:progress];
                           }

                           SuccBlock:^{
                           [TDeskMessageProgressManager.shareManager appendUploadProgress:uiMsg.msgID progress:100];
                            if (succ) {
                                 succ();
                             }
                             [TDeskMessageProgressManager.shareManager notifyMessageSendingResult:uiMsg.msgID result:TUIMessageSendingResultTypeSucc];
                           }
                           FailBlock:^(int code, NSString *desc) {
                             if (fail) {
                                 fail(code, desc);
                             }
                             [TDeskMessageProgressManager.shareManager notifyMessageSendingResult:uiMsg.msgID result:TUIMessageSendingResultTypeFail];
                           }];
                     }];
                   }];
}
...
- (void)sendPlaceHolderUIMessage:(TDeskMessageCellData *)placeHolderCellData {
...
- (void)removeUImsgAtIndex:(NSUInteger)index {
    if (index < self.uiMsgs.count) {
        TDeskMessageCellData *msg = self.uiMsgs[index];
...
- (void)replaceUIMsg:(TDeskMessageCellData *)cellData atIndex:(NSUInteger)index {
    if (index < self.uiMsgs.count) {
        TDeskMessageCellData *oldMsg = self.uiMsgs[index];
        if ([self.dataSource respondsToSelector:@selector(dataProvider:onRemoveHeightCache:)]) {
...
- (void)sendMessageReadReceiptAtIndexes:(NSArray *)indexes {
    ...
    for (NSNumber *i in indexes) {
        ...
        TDeskMessageCellData *data = self.uiMsgs_[[i intValue]];
...
- (nullable TDeskMessageCellData *)getSystemMsgFromDate:(NSDate *)date {
    if (self.msgForDate == nil || fabs([date timeIntervalSinceDate:self.msgForDate.timestamp]) > MaxDateMessageDelay) {
        TDeskMessageCellData *system = [self.class getSystemMsgFromDate:date];
        return system;
    }
    return nil;
}
...
+ (void)updateUIMsgStatus:(TDeskMessageCellData *)cellData uiMsgs:(NSArray *)uiMsgs {
    ...
    TDeskMessageCellData *data = uiMsgs[index];

    TDeskMessageCellData *lastData = nil;
    if (index >= 1) {
        lastData = uiMsgs[index - 1];
        if (![lastData isKindOfClass:[TDeskSystemMessageCellData class]]) {
            if ([lastData.identifier isEqualToString:data.identifier] &&
                ![data isKindOfClass:[TDeskSystemMessageCellData class]] &&(lastData.direction == data.direction)) {
    ...
    TDeskMessageCellData *nextData = nil;
...
- (void)onGroupInfoChanged:(NSString *)groupID changeInfoList:(NSArray<V2TIMGroupChangeInfo *> *)changeInfoList {
    ...
    for (V2TIMGroupChangeInfo *changeInfo in changeInfoList) {
        if (changeInfo.type == V2TIM_GROUP_INFO_CHANGE_TYPE_OWNER) {
            ...
            if ([ownerID isEqualToString:[TDeskLogin getUserID]]) {
...
- (void)onGrantAdministrator:(NSString *)groupID opUser:(V2TIMGroupMemberInfo *)opUser memberList:(NSArray <V2TIMGroupMemberInfo *> *)memberList {
    ...
    for (V2TIMGroupMemberInfo *changeInfo in memberList) {
        if (changeInfo.userID == [TDeskLogin getUserID]) {
...
@implementation TDeskMessageBaseDataProvider (IMSDK)
...
+ (NSString *)sendMessage:(V2TIMMessage *)message
    ...
    if (isSendPushInfo) {
        ...
        NSString *senderId = isGroup ? (groupID) : ([TDeskLogin getUserID]);
        senderId = senderId ?: @"";
        NSString *nickName = isGroup ? (conversationData.title) : ([TDeskLogin getNickName] ?: [TDeskLogin getUserID]);
        nickName = nickName ?: @"";
        ...
        entity.faceUrl = [TDeskLogin getFaceUrl] ?: @"";
        ...
        pushInfo.AndroidSound = TDeskConfig.defaultConfig.enableCustomRing ? @"private_ring" : nil;
...
+ (TDeskMessageCellData *__nullable)getCellData:(V2TIMMessage *)message {
...
+ (TDeskMessageCellData *)getRevokeCellData:(V2TIMMessage *)message {
    // subclass override required
    return nil;
}
