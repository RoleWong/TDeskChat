//
//  TDeskChatModifyMessageHelper.m
//  TUIChat
//
...
+ (V2TIMMessage *)resolveOriginCloudCustomData:(V2TIMMessage *)rootMsg simpleCurrentContent:(NSDictionary *)simpleCurrentContent {
    ...
    if (rootMsg.cloudCustomData) {
        NSDictionary *originDic = [TDeskTool jsonData2Dictionary:rootMsg.cloudCustomData];
    ...
    NSData *data = [TDeskTool dictionary2JsonData:messageReplies];
    if (data) {
...
@implementation TDeskChatModifyMessageHelper

+ (TDeskChatModifyMessageHelper *)defaultHelper {
    static dispatch_once_t onceToken;
    static TDeskChatModifyMessageHelper *config;
    dispatch_once(&onceToken, ^{
      config = [[TDeskChatModifyMessageHelper alloc] init];
    });
    return config;
}
