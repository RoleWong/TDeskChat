...
@interface TDeskEmojiConfig ()

@end

@implementation TDeskEmojiConfig

+ (void)load {
    NSLog(@"TDeskEmojiConfig load%@",[TDeskEmojiConfig defaultConfig]);
}
...
+ (id)defaultConfig {
    ...
    static TDeskEmojiConfig *config;
    dispatch_once(&onceToken, ^{
      config = [[TDeskEmojiConfig alloc] init];
    });
    return config;
}
...
@implementation TDeskEmojiConfig (defaultFace)
...
- (NSArray *)updateFaceGroups:(NSArray *)groups type:(TUIEmojiFaceType)type {
    if (groups.count) {
        ...
        TDeskFaceGroup *defaultFaceGroup = [self findFaceGroupAboutType:type];
        if (defaultFaceGroup) {
...
- (TDeskFaceGroup *)findFaceGroupAboutType:(TUIEmojiFaceType)type {
    // emoji group
...
- (void)addResourceToCache:(NSString *)path {
    [[TDeskImageCache sharedInstance] addResourceToCache:path];
}

- (void)addFaceToCache:(NSString *)path {
    [[TDeskImageCache sharedInstance] addFaceToCache:path];
}
@end
