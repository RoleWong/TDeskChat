...
#import "TDesk_TUIChatPopContextController.h"
...
#import "TDesk_TUIMessageReadViewController_Minimalist.h"
#import "TDesk_TUIOrderCell_Minimalist.h"
...
@interface TUIBaseMessageController_Minimalist () <TDeskMessageCellDelegate,
...
@property(nonatomic, strong) TDeskChatPopContextController *popAlertController;
...
- (void)registerEvents {
    [TDeskCore registerEvent:TUICore_TUIPluginNotify subKey:TUICore_TUIPluginNotify_WillForwardTextSubKey object:self];
    [TDeskCore registerEvent:TUICore_TUIPluginNotify subKey:TUICore_TUIPluginNotify_DidChangePluginViewSubKey object:self];
...
- (void)clearAndReloadCellOfData:(TDeskMessageCellData *)data {
    [self.messageCellConfig removeHeightCacheOfMessageCellData:data];
...
- (void)dataProvider:(TDeskMessageBaseDataProvider *)dataProvider didChangeTranslationData:(TDeskMessageCellData *)data {
    [self reloadAndScrollToBottomOfMessage:data.innerMessage.msgID];
}
...
- (void)showContextWindow:(TDeskMessageCell *)cell {
    CGRect frame = [UIApplication.sharedApplication.keyWindow convertRect:cell.container.frame fromView:cell];
    TDeskChatPopContextController *alertController = [[TDeskChatPopContextController alloc] init];
    ...
    __weak typeof(self) weakSelf = self;
    alertController.viewWillShowHandler = ^(TDeskMessageCell *_Nonnull alertView) {
...
- (void)configItems:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    ...
    TDeskChatPopContextExtionItem *copyItem = [self setupCopyAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *forwardItem = [self setupForwardAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *multiSelectItem = [self setupMultiSelectAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *referenceItem = [self setupReferenceAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *replyItem = [self setupReplyAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *revocationItem = [self setupRevocationAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *infoItem = [self setupInfoAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *deleteItem = [self setupDeleteAction:alertController targetCell:cell];

    TDeskChatPopContextExtionItem *audioPlaybackStyleItem = [self setupAudioPlaybackStyleAction:alertController targetCell:cell];
    
    TDeskChatPopContextExtionItem *groupPinItem = [self setupGroupPinAction:alertController targetCell:cell];
    if (isChatNoramlMessageOrCustomMessage) {
        ...
        if ([TDeskChatConfig defaultConfig].enablePopMenuReplyAction && (imMsg.status == V2TIM_MSG_STATUS_SEND_SUCC)) {
            [items addObject:replyItem];
        }
        if ([TDeskChatConfig defaultConfig].enablePopMenuReferenceAction && (imMsg.status == V2TIM_MSG_STATUS_SEND_SUCC)) {
            [items addObject:referenceItem];
        }
        [items addObject:deleteItem];
    ...
    NSArray<TUIExtensionInfo *> *infoArray =
        [TDeskCore getExtensionList:TUICore_TUIChatExtension_PopMenuActionItem_MinimalistExtensionID
    ...
    for (TUIExtensionInfo *info in infoArray) {
        if (info.text && info.icon && info.onClicked) {
            TDeskChatPopContextExtionItem *extension = [[TDeskChatPopContextExtionItem alloc] initWithTitle:info.text
                                                                                               markIcon:info.icon
                                                                                                 weight:info.weight
                                                                                      withActionHandler:^(TDeskChatPopContextExtionItem *action) {
    ...
    NSArray *sortResultArray = [items sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
      TDeskChatPopContextExtionItem *per1 = obj1;
      TDeskChatPopContextExtionItem *per2 = obj2;
    ...
    while (itemsRemaining) {
        ...
        TDeskChatPopContextExtionItem *lastItem = [subLogArr lastObject];
    ...
    if (allPageItemsArray.count != 1) {
        ...
        for (int pageIndex = 0; pageIndex < allPageItemsArray.count; pageIndex++) {
            ...
            TDeskChatPopContextExtionItem *moreItem =
                [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(More)
        ...
        // only one
        NSMutableArray<TDeskChatPopContextExtionItem *> *items = allPageItemsArray[0];
        TDeskChatPopContextExtionItem *lastItem = [items lastObject];
...
- (void)onJumpToRepliesDetailPage:(TDeskMessageCellData *)data {
    TDeskMessageCellData *copyData = [TDeskMessageDataProvider getCellData:data.innerMessage];
    @weakify(self);
    [self.messageDataProvider preProcessMessage:@[ copyData ]
                                       callback:^{
                                         dispatch_async(dispatch_get_main_queue(), ^{
                                           @strongify(self);
                                           TDeskMessageCell *cell = [[TDeskMessageCell alloc] init];
...
- (void)onJumpToMessageInfoPage:(TDeskMessageCellData *)data selectCell:(TDeskMessageCell *)cell {

    TDeskMessageCellData *alertViewCellData =  [TDeskMessageDataProvider getCellData:cell.messageData.innerMessage];;
    @weakify(self);
    [self.messageDataProvider preProcessMessage:@[alertViewCellData] callback:^{
        ...
        @weakify(self);
        readViewController.viewWillShowHandler = ^(TDeskMessageCell *_Nonnull alertView) {
          alertView.delegate = self;
        };
        readViewController.viewWillDismissHandler = ^(TDeskMessageCell * _Nonnull alertView) {
          self.hasCoverPage = NO;
        };
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([sender isKindOfClass:TUIReferenceMessageCell_Minimalist.class]) {
        ...
        TDeskReferenceMessageCellData *replyMsg = (TDeskReferenceMessageCellData *)replyMsgCell.data;
...
- (void)onDelete:(TDeskMessageCell *)cell {
    // subclass override
}

#pragma mark - config TUIChatPopContextExtionItems
- (TDeskChatPopContextExtionItem *)setupCopyAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(cell);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *copyItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Copy)
...
- (TDeskChatPopContextExtionItem *)setupForwardAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *forwardItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Forward)
...
- (TDeskChatPopContextExtionItem *)setupMultiSelectAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *multiSelectItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(MultiSelect)
...
- (TDeskChatPopContextExtionItem *)setupReferenceAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *referenceItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Quote)
...
- (TDeskChatPopContextExtionItem *)setupReplyAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *replyItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Reply)
...
- (TDeskChatPopContextExtionItem *)setupRevocationAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *revocationItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Recall)
...
- (TDeskChatPopContextExtionItem *)setupInfoAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(cell);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *infoItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Info)
...
- (TDeskChatPopContextExtionItem *)setupDeleteAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(cell);
    @weakify(alertController);
    TDeskChatPopContextExtionItem *deleteItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:TIMCommonLocalizableString(Delete)
...
- (TDeskChatPopContextExtionItem *)setupAudioPlaybackStyleAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    ...
    TDeskChatPopContextExtionItem *styleActionItem = nil;
    ...
    styleActionItem =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:title
                                                  markIcon:img
                                                    weight:11000
                                         withActionHandler:^(TDeskChatPopContextExtionItem *action) {
            @strongify(alertController);
...
- (TDeskChatPopContextExtionItem *)setupGroupPinAction:(TDeskChatPopContextController *)alertController targetCell:(TDeskMessageCell *)cell {
    @weakify(self);
    @weakify(cell);
    @weakify(alertController);
    ...
    TDeskChatPopContextExtionItem *groupPinAction =
        [[TDeskChatPopContextExtionItem alloc] initWithTitle:isPinned?
