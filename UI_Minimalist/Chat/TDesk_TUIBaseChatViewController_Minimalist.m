...
#import <TDeskCommon/TDesk_TUIFloatViewController.h>
#import <TDeskCommon/TDesk_TUIGroupAvatar+Helper.h>
...
#import "TDesk_TUIMessageController_Minimalist.h"
#import "TDesk_TUIMessageDataProvider.h"
#import "TDesk_TUIMessageMultiChooseView_Minimalist.h"
...
- (void)setupNavigator {
    ...
    UIImage *imgicon = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"vc_back")];
    ...
    NSArray<TUIExtensionInfo *> *extensionList = [TDeskCore getExtensionList:TUICore_TUIChatExtension_NavigationMoreItem_MinimalistExtensionID param:param];
...
- (void)updateSubTitleLabelText {
    if (![TDeskConfig defaultConfig].displayOnlineStatusIcon) {
...
- (void)setupTopExentsionView {
    ...
    [TDeskCore raiseExtension:TUICore_TUIChatExtension_ChatViewTopArea_MinimalistExtensionID parentView:gTopExentsionView param:param];
...
- (void)configHeadImageView:(TDeskChatConversationModel *)convData {
    /**
    ...
    if (convData.groupID.length > 0) {
         ...
         */
        convData.avatarImage = [TDeskGroupAvatar getNormalGroupCacheAvatar:convData.groupID groupType:convData.groupType];
    }
    ...
    [[RACObserve(convData, faceUrl) distinctUntilChanged] subscribeNext:^(NSString *faceUrl) {
      ...
      [TDeskGroupAvatar configAvatarByParam:param targetView:self.avatarView];
    }];
}
...
- (void)getUserOrFriendProfileVCWithUserID:(NSString *)userID succBlock:(void (^)(UIViewController *vc))succ failBlock:(nullable V2TIMFail)fail {
    ...
    [TDeskCore createObject:TUICore_TUIContactObjectFactory_Minimalist key:TUICore_TUIContactObjectFactory_GetUserOrFriendProfileVCMethod param:param];
}
...
- (BOOL)messageController:(TUIBaseMessageController_Minimalist *)controller willShowMenuInCell:(TDeskMessageCell *)cell {
...
- (TDeskMessageCellData *)messageController:(TUIBaseMessageController_Minimalist *)controller onNewMessage:(V2TIMMessage *)message {
    return nil;
}

- (TDeskMessageCell *)messageController:(TUIBaseMessageController_Minimalist *)controller onShowMessageData:(TDeskMessageCellData *)data {
    return nil;
}

- (void)messageController:(TUIBaseMessageController_Minimalist *)controller willDisplayCell:(TDeskMessageCell *)cell withData:(TDeskMessageCellData *)cellData {
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onSelectMessageAvatar:(TDeskMessageCell *)cell {
    NSString *userID = nil;
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onSelectMessageContent:(TDeskMessageCell *)cell {
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onSelectMessageMenu:(NSInteger)menuType withData:(TDeskMessageCellData *)data {
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onReEditMessage:(TDeskMessageCellData *)data {
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onSelectMessageWhenMultiCheckboxAppear:(TDeskMessageCellData *)data {
    if (_multiChooseView) {
...
- (void)selectTarget:(BOOL)mergeForward toForwardMessage:(NSArray<TDeskMessageCellData *> *)uiMsgs orForwardText:(NSString *)forwardText {
    __weak typeof(self) weakSelf = self;
    UIViewController *vc = (UIViewController *)[TDeskCore createObject:TUICore_TUIConversationObjectFactory_Minimalist
    ...
    vc.navigateValueCallback = ^(NSDictionary *param) {
      ...
      for (NSDictionary *selectItem in selectList) {
          TDeskChatConversationModel *model = [TDeskChatConversationModel new];
    ...
    TDeskFloatViewController *floatVC = [[TDeskFloatViewController alloc] init];
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onRelyMessage:(nonnull TDeskMessageCellData *)data {
    @weakify(self);
...
- (void)messageController:(TUIBaseMessageController_Minimalist *)controller onReferenceMessage:(TDeskMessageCellData *)data {
    @weakify(self);
    [self.inputController exitReplyAndReference:^{
      @strongify(self);
      NSString *desc = @"";
    ...
    for (TDeskChatConversationModel *conversation in conversations) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          if ([conversation.conversationID isEqualToString:self.conversationData.conversationID]) {
              ...
              message.needReadReceipt = self.conversationData.msgNeedReadReceipt && [TDeskChatConfig defaultConfig].msgNeedReadReceipt;
              [TDeskMessageBaseDataProvider sendMessage:message
