...
#import <TDeskCore/TDesk_UIColor+TUIHexColor.h>
#import "TDesk_TUIImageMessageCellData.h"
#import "TDesk_TUIMemberCellData.h"
#import "TDesk_TUIMemberCell_Minimalist.h"
...
@property(nonatomic, strong) TDeskMessageDataProvider *dataProvider;
...
@property(nonatomic, strong) TDeskMessageCell_Minimalist *alertView;
...
- (void)updateRootMsg {
    TDeskMessageCellData *data = self.alertViewCellData;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TDeskCommonCellData *data = [self members][indexPath.row];
    @weakify(self);
    if ([data isKindOfClass:TDeskMemberDescribeCellData.class]) {
        return;
    } else if ([data isKindOfClass:TDeskMemberCellData.class]) {
        if ([self isGroupMessageRead]) {
            if (indexPath.row >= [self members].count) {
                return;
            }
            TDeskMemberCellData *currentData = (TDeskMemberCellData *)data;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TDeskCommonCellData *data = [self members][indexPath.row];
    TDeskCommonTableViewCell *cell = nil;
...
- (NSArray *)members {
    ...
    if ([self isGroupMessageRead]) {
        if (self.readMembers.count > 0) {
            TDeskMemberDescribeCellData *describeCellData = [[TDeskMemberDescribeCellData alloc] init];
            describeCellData.title = TIMCommonLocalizableString(GroupReadBy);
            describeCellData.icon = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_status_all_people_read")];
            [dataArray addObject:describeCellData];

            for (V2TIMGroupMemberInfo *member in self.readMembers) {
                TDeskMemberCellData *data = [[TDeskMemberCellData alloc] initWithUserID:member.userID
        ...
        if (self.unreadMembers.count > 0) {
            ...
            describeCellData.icon = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_status_some_people_read")];
        ...
        TDeskMemberCellData *data = [[TDeskMemberCellData alloc] initWithUserID:self.cellData.innerMessage.userID
                                                                                         nickName:nil
