...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _voicePlay.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"voice_play")];
        [self.bubbleView addSubview:_voicePlay];

        self.voiceAnimations = [NSMutableArray array];
        for (int i = 0; i < 6; ++i) {
            UIImageView *animation = [[UIImageView alloc] init];
            animation.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"voice_play_animation")];
...
- (void)startAnimating {
    _voicePlay.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"voice_pause")];
}
...
+ (CGSize)getContentSize:(TDeskMessageCellData *)data {
    NSAssert([data isKindOfClass:TDeskVoiceMessageCellData.class], @"data must be kind of TDeskVoiceMessageCellData");
    TDeskVoiceMessageCellData *voiceCellData = (TDeskVoiceMessageCellData *)data;
