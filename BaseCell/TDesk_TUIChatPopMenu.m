...
@interface TDeskChatPopMenu () <UIGestureRecognizerDelegate,V2TIMAdvancedMsgListener>
...
@property(nonatomic, strong) TDeskChatPopActionsView *actionsView;

@property(nonatomic, assign) BOOL hasEmojiView;

@end

@implementation TDeskChatPopMenu
...
- (BOOL)isAddEmojiView {
    return self.hasEmojiView && [TDeskChatConfig defaultConfig].enablePopMenuEmojiReactAction;
}
...
- (void)updateActionByRank {
    NSArray *ageSortResultArray = [self.actions sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
      TDeskChatPopMenuAction *per1 = obj1;
      TDeskChatPopMenuAction *per2 = obj2;
...
- (void)prepareContainerView {
    ...
    int i = 0;
    for (TDeskChatPopMenuAction *action in self.actions) {
