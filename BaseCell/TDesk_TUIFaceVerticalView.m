...
@interface TDeskFaceVerticalView () <UICollectionViewDelegate,
...
@implementation TDeskFaceVerticalView
...
- (void)setupViews {
    ...
    _faceFlowLayout = [[TDeskCollectionRTLFitFlowLayout alloc] init];
    ...
    [_faceCollectionView registerClass:[TDeskFaceCell class] forCellWithReuseIdentifier:TFaceCell_ReuseId];
...
- (void)setData:(NSMutableArray *)data {
    ...
    for (NSInteger groupIndex = 0; groupIndex < _faceGroups.count; ++groupIndex) {
        TDeskFaceGroup *group = _faceGroups[groupIndex];
    ...
    for (NSInteger curSection = 0; curSection < _sectionCount; ++curSection) {
        ...
        TDeskFaceGroup *face = _faceGroups[groupIndex.integerValue];
    ...
    TDeskFaceGroup *group = _faceGroups[0];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TDeskFaceCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:TFaceCell_ReuseId forIndexPath:indexPath];
    ...
    if (index.integerValue < group.faces.count) {
        TDeskFaceCellData *data = group.faces[index.integerValue];
        [cell setData:data];
    } else {
        [cell setData:nil];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    int groupIndex = [_groupIndexInSection[indexPath.section] intValue];
    TDeskFaceGroup *faces = _faceGroups[groupIndex];
    NSNumber *index = [_itemIndexs objectForKey:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView
    ...
    TDeskFaceGroup *group = _faceGroups[section];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    TDeskFaceGroup *group = _faceGroups[indexPath.section];
    if (group.groupName.length > 0 ){
...
- (void)showDisplayView:(CGFloat)display_x
             ...
             targetView:(UIView *)targetView
               faceCell: (TDeskFaceCell *)faceCell{
