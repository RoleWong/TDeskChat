...
@implementation TDeskVoiceMessageCell
...
+ (CGSize)getContentSize:(TDeskMessageCellData *)data {
    ...
    if (voiceCellData.direction == MsgDirectionIncoming) {
        bubbleWidth = MAX(bubbleWidth, [TDeskBubbleMessageCell incommingBubble].size.width);
        bubbleHeight = voiceCellData.voiceImage.size.height + 2 * voiceCellData.voiceTop;  //[TDeskBubbleMessageCellData incommingBubble].size.height;
    } else {
        bubbleWidth = MAX(bubbleWidth, [TDeskBubbleMessageCell outgoingBubble].size.width);
        bubbleHeight = voiceCellData.voiceImage.size.height + 2 * voiceCellData.voiceTop;  // [TDeskBubbleMessageCellData outgoingBubble].size.height;
    }
