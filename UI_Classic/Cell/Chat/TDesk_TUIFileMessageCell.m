...
@interface TDeskIFileMessageCell () <V2TIMSDKListener, TDeskMessageProgressManagerDelegate>
...
@implementation TDeskIFileMessageCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.securityStrikeView = [[TDeskSecurityStrikeView alloc] init];
        ...
        _image.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath(@"msg_file_p")];
...
- (void)fillWithData:(TDeskFileMessageCellData *)data {
    ...
    _image.image = [[TDeskImageCache sharedInstance] getResourceFromCache:[self getImagePathByCurrentFileType:data.fileName.pathExtension]];
    @weakify(self);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      @strongify(self);
      NSInteger uploadProgress = [TDeskMessageProgressManager.shareManager uploadProgressForMessage:self.fileData.msgID];
      NSInteger downloadProgress = [TDeskMessageProgressManager.shareManager downloadProgressForMessage:self.fileData.msgID];
...
- (UIImage *)getErrorBubble {
    if (self.messageData.direction == MsgDirectionIncoming) {
        return TDeskBubbleMessageCell.incommingErrorBubble;
    } else {
        return TDeskBubbleMessageCell.outgoingErrorBubble;
    }
}
...
+ (CGSize)getFileContentSize:(TDeskMessageCellData *)data {
