...
@implementation TDeskTextMessageCell
...
+ (CGSize)getContentSize:(TDeskMessageCellData *)data {
    ...
    if (textCellData.direction == MsgDirectionIncoming) {
        height = MAX(height, TDeskBubbleMessageCell.incommingBubble.size.height);
    } else {
        height = MAX(height, TDeskBubbleMessageCell.outgoingBubble.size.height);
    }
...
@implementation TDeskTextMessageCell (TUILayoutConfiguration)
