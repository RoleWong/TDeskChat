...
@interface TDeskMenuView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@property(nonatomic, strong) NSMutableArray<TDeskMenuCellData *> *data;
@end

@implementation TDeskMenuView
...
- (void)setupViews {
    ...
    _menuFlowLayout = [[TDeskCollectionRTLFitFlowLayout alloc] init];
    ...
    [_menuCollectionView registerClass:[TDeskMenuCell class] forCellWithReuseIdentifier:TMenuCell_ReuseId];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row % 2 == 0) {
        TDeskMenuCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:TMenuCell_ReuseId forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSInteger i = 0; i < _data.count; ++i) {
        TDeskMenuCellData *data = _data[i];
