...
@interface TDeskInputController () <TDeskInputBarDelegate, TDeskMenuViewDelegate, TDeskFaceViewDelegate, TDeskMoreViewDelegate>
@property(nonatomic, assign) InputStatus status;
@property(nonatomic, assign) CGRect keyboardFrame;

@property(nonatomic, copy) void (^modifyRootReplyMsgBlock)(TDeskMessageCellData *);
@end

@implementation TDeskInputController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setupViews];
}
...
- (void)setupViews {
    ...
    _inputBar = [[TDeskInputBar alloc] initWithFrame:CGRectZero];
...
- (void)inputBarDidTouchVoice:(TDeskInputBar *)textView {
...
- (void)inputBarDidTouchMore:(TDeskInputBar *)textView {
...
- (void)inputBarDidTouchFace:(TDeskInputBar *)textView {
    if ([TDeskIMConfig defaultConfig].faceGroups.count == 0) {
        return;
    }
...
- (void)inputBarDidTouchKeyboard:(TDeskInputBar *)textView {
    if (_status == Input_Status_Input_More) {
        [self hideMoreAnimation];
    }
    if (_status == Input_Status_Input_Face) {
...
- (void)inputBar:(TDeskInputBar *)textView didChangeInputHeight:(CGFloat)offset {
    if (_status == Input_Status_Input_Face) {
        [self showFaceAnimation];
    } else if (_status == Input_Status_Input_More) {
        [self showMoreAnimation];
...
- (void)inputBar:(TDeskInputBar *)textView didSendText:(NSString *)text {
    /**
...
- (void)inputMessageStatusChanged:(NSNotification *)noti {
    ...
    TDeskMessageCellData *msg = userInfo[@"msg"];
    long status = [userInfo[@"status"] intValue];
    if ([msg isKindOfClass:TDeskMessageCellData.class] && status == Msg_Status_Succ) {
...
- (void)appendReplyDataIfNeeded:(V2TIMMessage *)message {
    if (self.replyData) {
        ...
        if (parentMsg.cloudCustomData) {
            NSDictionary *originDic = [TDeskTool jsonData2Dictionary:parentMsg.cloudCustomData];
        ...
        NSData *data = [TDeskTool dictionary2JsonData:cloudResultDic];
        if (data) {
        ...
        self.modifyRootReplyMsgBlock = ^(TDeskMessageCellData *cellData) {
...
- (void)modifyRootReplyMsgByID:(NSString *)messageRootID currentMsg:(TDeskMessageCellData *)messageCellData {
    ...
    if (messageRootID) {
        [TDeskChatDataProvider findMessages:@[ messageRootID ]
                                 callback:^(BOOL succ, NSString *_Nonnull error_message, NSArray *_Nonnull msgs) {
                                   if (succ) {
                                       if (msgs.count > 0) {
                                           V2TIMMessage *rootMsg = msgs.firstObject;
                                           [[TDeskChatModifyMessageHelper defaultHelper] modifyMessage:rootMsg simpleCurrentContent:simpleCurrentContent];
                                       }
                                   }
                                 }];
    }
...
- (void)inputBar:(TDeskInputBar *)textView didSendVoice:(NSString *)path {
...
- (void)inputBarDidDeleteBackward:(TDeskInputBar *)textView {
...
- (void)faceView:(TDeskFaceView *)faceView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    TDeskFaceGroup *group = faceView.faceGroups[indexPath.section];
    TDeskFaceCellData *face = group.faces[indexPath.row];
...
- (TDeskFaceSegementScrollView *)faceSegementScrollView {
    if(!_faceSegementScrollView) {
        _faceSegementScrollView = [[TDeskFaceSegementScrollView alloc]
            initWithFrame:CGRectMake(0,
                                     _inputBar.frame.origin.y + _inputBar.frame.size.height,
                                     self.view.frame.size.width,
                                     TFaceView_Height)];
        [_faceSegementScrollView setItems:[TDeskIMConfig defaultConfig].faceGroups delegate:self];
    }
    return _faceSegementScrollView;
}
- (TDeskMoreView *)moreView {
    if (!_moreView) {
        _moreView =
            [[TDeskMoreView alloc] initWithFrame:CGRectMake(0,
                                                          _inputBar.frame.origin.y + _inputBar.frame.size.height,
...
- (TDeskMenuView *)menuView {
    if (!_menuView) {
        _menuView = [[TDeskMenuView alloc]
        ...
        TDeskIMConfig *config = [TDeskIMConfig defaultConfig];
        NSMutableArray *menus = [NSMutableArray array];
        for (NSInteger i = 0; i < config.faceGroups.count; ++i) {
            TDeskFaceGroup *group = config.faceGroups[i];
...
- (TDeskReplyPreviewBar *)replyPreviewBar {
    if (_replyPreviewBar == nil) {
        _replyPreviewBar = [[TDeskReplyPreviewBar alloc] init];
        __weak typeof(self) weakSelf = self;
...
- (TDeskReferencePreviewBar *)referencePreviewBar {
    if (_referencePreviewBar == nil) {
        _referencePreviewBar = [[TDeskReferencePreviewBar alloc] init];
        __weak typeof(self) weakSelf = self;
