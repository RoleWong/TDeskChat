//
//  TDeskMessageReadViewController.m
//  TUIChat
//
...
@implementation TDeskMessageReadSelectView
...
@interface TDeskMessageReadViewController () <UITableViewDelegate, UITableViewDataSource, TDeskMessageReadSelectViewDelegate>
...
@implementation TDeskMessageReadViewController
...
- (void)setupSelectView {
    ...
    for (NSNumber *tag in dataDict) {
        ...
        TDeskMessageReadSelectView *selectView = [[TDeskMessageReadSelectView alloc] initWithTitle:data[@"title"]
...
- (void)setupTableView {
    ...
    [_tableView registerClass:[TDeskMemberCell class] forCellReuseIdentifier:kMemberCellReuseId];
...
- (void)getReadMembersWithCompletion:(void (^)(int code, NSString *desc, NSArray *members, BOOL isFinished))completion {
    @weakify(self);
    [TDeskMessageDataProvider getReadMembersOfMessage:self.cellData.innerMessage
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TDeskMemberCell *cell = [tableView dequeueReusableCellWithIdentifier:kMemberCellReuseId forIndexPath:indexPath];
    cell.changeColorWhenTouched = YES;

    TDeskMemberCellData *data;
    if ([self isGroupMessageRead]) {
        if (indexPath.row >= [self members].count) {
            return nil;
        }
        V2TIMGroupMemberInfo *member = [self members][indexPath.row];
        data = [[TDeskMemberCellData alloc] initWithUserID:member.userID
...
#pragma mark - TDeskMessageReadSelectViewDelegate
- (void)messageReadSelectView:(TDeskMessageReadSelectView *)view didSelectItemTag:(TUIMessageReadViewTag)tag {
    for (TDeskMessageReadSelectView *view in self.selectViewsDict.allValues) {
...
- (id)content {
    ...
    NSMutableString *content = [NSMutableString stringWithString:[TDeskMessageDataProvider getDisplayString:msg]];
    switch (msg.elemType) {
        case V2TIM_ELEM_TYPE_IMAGE: {
            TDeskImageMessageCellData *data = (TDeskImageMessageCellData *)self.cellData;
            return data.thumbImage;
        }
        case V2TIM_ELEM_TYPE_VIDEO: {
            TDeskVideoMessageCellData *data = (TDeskVideoMessageCellData *)self.cellData;
            return data.thumbImage;
        }
