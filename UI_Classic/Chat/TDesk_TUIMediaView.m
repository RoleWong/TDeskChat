...
#import "TDesk_TUIImageCollectionCell.h"
#import "TDesk_TUIMessageMediaDataProvider.h"
#import "TDesk_TUIVideoCollectionCell.h"

#define ANIMATION_TIME 0.2

@interface TDeskMediaView () <UICollectionViewDelegate,
                            UICollectionViewDataSource,
                            UICollectionViewDelegateFlowLayout,
                            UIScrollViewDelegate,
                            TDeskMediaCollectionCellDelegate>
@property(strong, nonatomic) TDeskMessageMediaDataProvider *dataProvider;
...
- (void)setupViews {
    ...
    TDeskCollectionRTLFitFlowLayout *menuFlowLayout = [[TDeskCollectionRTLFitFlowLayout alloc] init];
    ...
    [self.menuCollectionView registerClass:[TDeskImageCollectionCell class] forCellWithReuseIdentifier:TImageMessageCell_ReuseId];
    [self.menuCollectionView registerClass:[TDeskVideoCollectionCell class] forCellWithReuseIdentifier:TVideoMessageCell_ReuseId];
...
- (void)setCurMessage:(V2TIMMessage *)curMessage {
    ...
    self.dataProvider = [[TDeskMessageMediaDataProvider alloc] initWithConversationModel:model];
    ...
    [RACObserve(self.dataProvider, medias) subscribeNext:^(NSArray *x) {
      ...
      for (int i = 0; i < self.dataProvider.medias.count; i++) {
          TDeskMessageCellData *data = self.dataProvider.medias[i];
...
- (void)setCurMessage:(V2TIMMessage *)curMessage allMessages:(NSArray<V2TIMMessage *> *)allMessages {
    ...
    for (V2TIMMessage *message in allMessages) {
        TDeskMessageCellData *data = [TDeskMessageMediaDataProvider getMediaCellData:message];
        if (data) {
            [medias addObject:data];
        }
    }

    self.dataProvider = [[TDeskMessageMediaDataProvider alloc] initWithConversationModel:nil];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TDeskMessageCellData *data = self.dataProvider.medias[indexPath.row];
    TDeskMediaCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:data.reuseId forIndexPath:indexPath];
    if (cell) {
        cell.delegate = self;
        [cell fillWithData:data];
        if ([cell isKindOfClass:[TDeskVideoCollectionCell class]]) {
            TDeskVideoCollectionCell *videoCell = (TDeskVideoCollectionCell *)cell;
            [videoCell reloadAllView];
        }
        else if ([cell isKindOfClass:[TDeskImageCollectionCell class]]) {
            TDeskImageCollectionCell *imageCell =  (TDeskImageCollectionCell *)cell;
...
#pragma mark TDeskMediaCollectionCellDelegate
